...
xml_root = r"D:\Infomize\AI\Annotations\entities\For_training\7-Nov\test_output\Annotations"
new_xml_root = r"D:\Infomize\AI\Annotations\entities\For_training\7-Nov\test_output\new_annot"
image_root = r"D:\Infomize\AI\Annotations\entities\For_training\7-Nov\test_output\JPEGImages"
...
def check_hw():
    ...
    for n,xml_name in enumerate(xml_name_list):
        xml_path = os.path.join(xml_root, xml_name)
        tree = ET.parse(xml_path)
        root = tree.getroot()
        size = root.find("size")
    ...
    for n,xml_name in enumerate(xml_name_list):
        ...
        for obj in root.findall("object"):
            ...
            if bbox[0] >= bbox[2] or bbox[1] >= bbox[3]:
                ...
                print('improper coordinates:', bbox, xml_name)
            ...
            if bbox[3] > h or bbox[2] > w:
                ...
                print("coordinates exceeds image size", bbox, h, w, xml_name)
                # bboxes = np.array([bbox])
                # mmcv.imshow_det_bboxes(img, bboxes, labels=np.array(["h"]))
