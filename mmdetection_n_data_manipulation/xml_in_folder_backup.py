...
for file in os.listdir(srcdir):
	try:
		src2=os.listdir(os.path.join(srcdir,file+"/images"))
		for element in src2:
			if element.endswith('.xml'):
				...
				if not os.path.exists(desdir):
					os.makedirs(desdir)
				shutil.copy(os.path.join(os.path.join(srcdir,file+"/images"),os.path.splitext(element)[0]+'.jpg'),desdir)
			'''
			if element.endswith('.jpg'):
				desdir = r'D:\Infomize\AI\Annotations\table\test_anjali\JPEGImages'
				if not os.path.exists(desdir):
					os.makedirs(desdir)
				shutil.copy(os.path.join(os.path.join(srcdir,file+"/images"),element),desdir)	
			'''	
